...
<div class="breadcrumbs" id="breadcrumbs">
	...
	<ul class="breadcrumb">
		...
		<li class="active">
			<a href="/admin/switch_router/stock_out">Stock Out</a>
		</li>
	</ul>
</div>
{{ Form::open( array( 'route' => array('admin.switch_router.create_stock_out'), 'method' => 'POST','class' => 'form-horizontal validate-form', 'role' => 'form')  ) }}
	<div class="row">
		<div class="col-xs-12">
			...
			<div class="form-group">
                ...
                <div class="col-xs-12 col-sm-9">
                    <select  name="employee" class="select2 employee"
                            ...
                            <option value="{{$value->employee_identity}}">{{$value->name}}-{{$value->employee_identity}}</option>
                   			@endforeach
                    </select>
                </div>
        	</div>
			...
			<div class="form-group final_c">
				{{ Form::label('TOTAL COUNT','TOTAL COUNT', array('class' => 'col-sm-3 control-label no-padding-right'))}}
				<div class="col-sm-9">
					{{ Form::text('final_count','', array('class' => 'col-xs-10 col-sm-5 final_count')) }}
				</div>
			</div>
			<div class="form-group final_count1">
				{{ Form::label('TOTAL COUNT','TOTAL COUNT', array('class' => 'col-sm-3 control-label no-padding-right'))}}
				<div class="col-sm-9">
					{{ Form::text('final_count1','', array('class' => 'col-xs-10 col-sm-5 final_count1','onchange'=>'TotalMeterCalc(1)')) }}
				</div>
			</div>
			
			{{ Form::hidden('c_id','', array('class' => 'col-xs-10 col-sm-5 c_id')) }}
			<div class="meter">
				<div class="col-sm-12">
				...
				<div class="form-group col-sm-12">
						<div class="col-sm-2">
							<input type="text" name='meter_drum_no[]' value="1"  placeholder='Drum No' class="form-control no" readonly="true"/>
						</div>
						<div class="col-sm-2">
							<input type="text" name='drum_no[]'  placeholder='Drum No' class="form-control drum_no1"/>
						</div>	
						<div class="col-sm-2">		
			...
			<div class="count">
				...
				<div class="form-group col-sm-12">
						...
						<div class="col-sm-2">		
							<input type="text" name='srl_no[]' placeholder='Srl No' class="form-control srl_no1" onchange="TotalMeterCalc(1)"/>
						</div>
						<div class="col-sm-2">
							<input type="text" name='mac_address[]' placeholder='Mac Address' class="form-control mac_address1"/>
						</div>
						<div class="col-sm-2">
							<input type="text" name='count[]' placeholder='Select Material Count' class="form-control m_count1" onchange="TotalMeterCalc(1)" readonly="true"/>
						</div>
...
function TotalMeterCalc(x){
	...
	var drum_no=$('.drum_no'+x).val();
	var srl_no=$('.srl_no'+x).val();
	var final_c=$('.final_count'+x).val();
	var material = $('select.material_type').val();
	var type = $('select.type').val();

		 $.ajax({
            url : '/admin/switch_router/check_stock',
            type : 'get',
            data : {'material':material,'type':type,'start':start_meter,'end':end_meter,'final_c':final_c,'drum_no':drum_no,'srl_no':srl_no},
           success : function(data) {
                   if (data["found"] == "false") {
                    alert(data['stock']);
                	} else {
                   		if(start_meter.length!=0 && end_meter.length!=0 && drum_no.length!=0){
	...
	function TotalCheck(x,m) {
       ...
       if(m<x){
       		$('.t_meter'+x).val(0);
       		alert("Meter Range Not Available In the Drum !!!!!");
       }
    }
