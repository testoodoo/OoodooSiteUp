<div class="form-group">
	...
	<div class="col-sm-9">
		{{ Form::text('account_id', $adjustment->account_id , array('class' => 'col-xs-10 col-sm-5 required account_id')) }}
	</div>
</div>
<div class="form-group">
	{{ Form::label('amount','Amount', array('class' => 'col-sm-3 control-label no-padding-right'))}}
	<div class="col-sm-9">
		{{ Form::text('amount', $adjustment->amount , array('class' => 'col-xs-10 col-sm-5 required digits')) }}
	</div>
</div>
<div class="form-group">
	{{ Form::label('for_month','For Month', array('class' => 'col-sm-3 control-label no-padding-right'))}}
	<div class="col-sm-9">
		{{ Form::text('for_month', $adjustment->for_month , array('class' => 'col-xs-10 col-sm-5 date-picker for_month ','onchange' =>'datevalue()','required ')) }}
	</div>
</div>
<div class="form-group">
	{{ Form::label('remarks','Remarks', array('class' => 'col-sm-3 control-label no-padding-right'))}}
	<div class="col-sm-9">
		{{ Form::textarea('remarks', $adjustment->remarks , array('class' => 'col-xs-10 col-sm-5 required')) }}
	</div>
</div>
<div class="form-group">
	{{ Form::label('is_considered','Is considered', array('class' => 'col-sm-3 control-label no-padding-right'))}}
	<div class="col-sm-9">
		@if( $adjustment->is_considered == 1)
			{{ Form::checkbox('is_considered', $adjustment->is_considered, null, array('checked' => 'checked','disabled')) }}
		@else
